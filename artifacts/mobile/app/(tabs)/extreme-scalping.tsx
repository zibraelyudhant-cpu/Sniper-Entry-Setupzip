...
function AnalisaTab({ colors, initialSymbol, initialMode, pinnedData }: { colors: ReturnType<typeof useColors>; initialSymbol?: string; initialMode?: 'quant' | 'sniper'; pinnedData?: ExtremeScalpingResult | null }) {
  const insets = useSafeAreaInsets();
  ...
  const handleSaveJournal = useCallback(async () => {
    ...
    const entry: JournalEntry = {
      ...
      orderType: 'limit', // Quant & Sniper (trend/range mode) dua-duanya basis breakout/rejection+retest, selalu LIMIT
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      {activeTab === 'scan'
        ...
        : activeTab === 'analisa'
        ? <AnalisaTab colors={colors} initialSymbol={pinnedCoin?.symbol} initialMode={pinnedCoin?.mode as 'quant' | 'sniper' | undefined} pinnedData={pinnedCoin} />
        : <MenuJournalSummary sourceMenu="Extreme Scalping" accentColor={ACCENT} />
      }
    </View>
  );
}
